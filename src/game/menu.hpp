...
// mouse sensitivity bar, plus Restart / Resume / Quit buttons.
//
...
class Menu {
public:
    enum Item { Volume = 0, Resolution = 1, Sensitivity = 2, Restart = 3, Resume = 4, Quit = 5, Count = 6 };
    ...
    void render(Renderer& r) const {
        ...
        static const char* kLabels[3] = {"MASTER VOLUME", "RESOLUTION", "MOUSE SENSITIVITY"};
        for (int i = 0; i < 3; ++i) {
...
private:
    struct Layout {
        static constexpr float pw = 520, ph = 450;
        int winW = 0, winH = 0;
        float px = 0, py = 0;
        float rowY[3]{};
    ...
    void computeLayout(int w, int h) {
        ...
        float y0 = L.py + 92;
        for (int i = 0; i < 3; ++i) L.rowY[i] = y0 + float(i) * 54;
    ...
    Layout layout_;
    char volText_[8]{}, sensText_[8]{}, resText_[24]{};
